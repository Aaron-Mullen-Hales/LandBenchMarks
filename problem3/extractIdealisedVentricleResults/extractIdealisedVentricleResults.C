...
#   include "createMesh.H"
	...
	// fallback if seeded search fails
	if (midLineCellIDs[I] == -1)
	  {
	    midLineCellIDs[I] = searchEngine.findCell(midLine[I]);
	  }
    ...
    if (min(midLineCellIDs) == -1)
      {
	FatalError
	  << "Could not find containing cells on the reference mid line"
	  << abort(FatalError);
      }


    // Interpolate displacement at the actual reference midLine points
    interpolationCellPoint<vector> DInterp(D);

    vectorField midLineDisp(midLine.size(), vector::zero);
    vectorField midLineCoord(midLine.size(), vector::zero);

    forAll(midLine, pI)
      {
	const label cellID = midLineCellIDs[pI];

	midLineDisp[pI] = DInterp.interpolate(midLine[pI], cellID);
	midLineCoord[pI] = midLine[pI] + midLineDisp[pI];
      }
